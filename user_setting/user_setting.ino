...
public:
  LGFX(void)
  {
    ...
    { // 디스플레이 패널 제어 설정을 합니다.
      ...
      cfg.invert           = false;  // 패널의 밝기가 반전되는 경우 true로 설정
...
// 준비한 클래스의 인스턴스를 생성합니다.
LGFX display;

void setup(void)
{
  // SPI 버스와 패널을 초기화하면 사용할 준비가 됩니다.
  Serial.begin(115200);
  display.init();

  display.setTextSize((std::max(display.width(), display.height()) + 255) >> 8);

  // 터치가 사용 가능한 경우 캘리브레이션을 수행합니다. (생략 가능)
  if (display.touch())
  {
    ...
    display.drawString("touch plz.", display.width()>>1, display.height() >> 1);
    display.setTextDatum(textdatum_t::top_left);

    // 터치를 사용하는 경우, 캘리브레이션을 수행합니다. 화면의 네 모서리에 표시되는 화살표의 끝을 차례로 터치하세요.
